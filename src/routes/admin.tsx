...
/* ---- Showcase Gallery Panel (Culture & Classes Slider) ---- */
function ShowcaseGalleryPanel() {
  const qc = useQueryClient();
  const [uploading, setUploading] = useState(false);
  
  // Fetch showcase items with 'as any' to bypass local type checks
  const { data: images = [] } = useQuery({ 
    queryKey: ["admin-showcase-gallery"], 
    queryFn: async () => {
      const { data, error } = await (supabase.from("showcase_gallery_images" as any) as any)
        .select("*")
        .order("sort_order", { ascending: true });
      if (error) throw error;
      return Promise.all((data || []).map(async (img: any) => ({ ...img, signedUrl: await signedUrl(img.image_url) })));
    } 
  });

  const refresh = () => {
    qc.invalidateQueries({ queryKey: ["admin-showcase-gallery"] });
    qc.invalidateQueries({ queryKey: ["showcase-gallery"] });
  };
  ...
  async function onUpload(files: FileList | null) {
    ...
    setUploading(true);
    try {
      const maxOrder = Math.max(0, ...images.map((i: any) => i.sort_order));
      for (let i = 0; i < files.length; i++) {
        const file = files[i];
        const ext = file.name.split(".").pop() ?? "jpg";
        const path = `showcase/${Date.now()}-${i}.${ext}`;
        const { error: upErr } = await supabase.storage.from(BUCKET).upload(path, file);
        if (upErr) throw upErr;
        const { error: dbErr } = await (supabase.from("showcase_gallery_images" as any) as any).insert({ 
          image_url: path, 
          storage_path: path, 
      ...
      toast.success(`Uploaded ${files.length} showcase image${files.length > 1 ? "s" : ""}`);
      refresh();
    } catch (err) {
      toast.error((err as Error).message);
    } finally {
  ...
  async function onDelete(id: string, path: string) {
    ...
    try {
      await supabase.storage.from(BUCKET).remove([path]);
      const { error } = await (supabase.from("showcase_gallery_images" as any) as any).delete().eq("id", id);
      if (error) throw error;
      toast.success("Deleted");
      refresh();
    } catch (err) {
  ...
  async function move(index: number, dir: -1 | 1) {
    const other = index + dir;
    if (other < 0 || other >= images.length) return;
    const a: any = images[index], b: any = images[other];
    try {
      await (supabase.from("showcase_gallery_images" as any) as any).update({ sort_order: b.sort_order }).eq("id", a.id);
      await (supabase.from("showcase_gallery_images" as any) as any).update({ sort_order: a.sort_order }).eq("id", b.id);
      refresh();
    } catch (err) {
      toast.error((err as Error).message);
    }
  ...
  return (
    <section className="rounded-3xl border border-white/10 bg-card p-6 md:p-8">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h2 className="font-display text-xl font-extrabold">Culture & Classes Showcase Slider</h2>
          <p className="mt-1 text-sm text-muted-foreground">Manage images shown in the "A Glimpse Into Our Culture & Classes" slider.</p>
        </div>
        <label className={`inline-flex items-center gap-2 rounded-full btn-cta px-5 py-2.5 text-sm font-semibold cursor-pointer ${uploading ? "opacity-60 pointer-events-none" : ""}`}>
          {uploading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Upload className="h-4 w-4" />} Upload showcase images
          <input type="file" accept="image/*" multiple className="hidden" onChange={(e) => onUpload(e.target.files)} />
        </label>
      </div>
      ...
      {images.length === 0 ? (
        <div className="mt-6 rounded-2xl border border-dashed border-white/15 p-10 text-center text-muted-foreground text-sm">
          <ImageIcon className="h-8 w-8 mx-auto opacity-50" />
          <p className="mt-2">No showcase images yet. Upload some to populate the slider.</p>
        </div>
      ) : (
        <ul className="mt-6 grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {images.map((img: any, i: number) => (
            <li key={img.id} className="group rounded-2xl overflow-hidden border border-white/10 bg-navy-deep">
              <div className="aspect-[4/5] bg-navy-deep">
                {img.signedUrl && <img src={img.signedUrl} alt="" className="h-full w-full object-cover" />}
              </div>
              <div className="p-3 flex items-center justify-between gap-2">
                <span className="text-xs text-muted-foreground">Position {i + 1}</span>
